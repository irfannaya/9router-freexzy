...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  try {
    ...
    const availableConnections = connections.filter(c => {
      ...
      // Antigravity: skip if live quota exhausted for this model (only if within 5m to prevent weekly 7d block)
      if (isAntigravity && model && antigravityQuotaCache) {
        const quota = antigravityQuotaCache.get(c.id)?.[model];
        if (quota && quota.remainingPercentage <= 0 && quota.resetAt) {
          const resetTime = new Date(quota.resetAt).getTime();
          const now = Date.now();
          if (resetTime > now && resetTime - now <= 5 * 60 * 1000) {
    ...
    if (availableConnections.length === 0) {
      if (connections.length > 0 && excludeSet.size === 0) {
        log.warn("AUTH", `${provider} | all ${connections.length} accounts locked for ${model || "all"} — fail-soft fallback: trying first connection ${connections[0].id?.slice(0, 8)}`);
        availableConnections.push(connections[0]);
      } else {
